...
export default function CompleteProfile() {
  ...
  const [clothes, setClothes] = useState<Array<{ file: File; kind: 'top' | 'bottom' }>>([]);
  const [selectedKnobPreset, setSelectedKnobPreset] = useState<string | null>(null);
  ...
  async function onSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (doorKnobFile) {
        knobUrl = await uploadFile(`users/${user.uid}/knob_${Date.now()}`, doorKnobFile);
      }

      // Upload clothes
      const clothingUploaded: ClothingItem[] = [];
      for (const c of clothes) {
        const url = await uploadFile(`users/${user.uid}/clothes/${Date.now()}_${c.file.name}`, c.file);
      ...
      <form className="stack" onSubmit={onSave}>
        ...
        <div className="row">
          <div style={{ flex: 1 }}>
            ...
            <input className="input" value={institute} onChange={e => setInstitute(e.target.value)} required />
          </div>
          <div style={{ flex: 1 }}>
        ...
        <div className="row">
          <div style={{ flex: 1 }}>
            ...
            <input className="input" value={bloodGroup} onChange={e => setBloodGroup(e.target.value)} required />
          </div>
          <div style={{ flex: 1 }}>
            <label className="label">Aesthetic</label>
            <select className="input" value={aesthetic} onChange={e => setAesthetic(e.target.value as any)} required>
        ...
        <div className="row">
          <div style={{ flex: 1 }}>
            <label className="label">Door knob (profile picture, circle crop)</label>
            <input className="input" type="file" accept="image/*" onChange={e => setDoorKnobFile(e.target.files?.[0] ?? null)} />
          </div>
          <div style={{ flex: 1 }}>
            <label className="label">Select a door knob preset</label>
            <select className="input" value={selectedKnobPreset ?? ''} onChange={e => setSelectedKnobPreset(e.target.value || null)}>
        ...
        <div className="row">
          <div style={{ flex: 1 }}>
            ...
            <input className="input" type="file" accept="image/*" multiple onChange={e => addClothing(e.target.files, 'top')} />
          </div>
          <div style={{ flex: 1 }}>
            <label className="label">Upload clothing (bottom)</label>
            <input className="input" type="file" accept="image/*" multiple onChange={e => addClothing(e.target.files, 'bottom')} />
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 8 }}>
          {clothes.map((c, idx) => (
            <div key={idx} className="card" style={{ padding: 6 }}>
              <div style={{ fontSize: 12, color: 'var(--color-muted)' }}>{c.kind}</div>
              <div style={{ width: '100%', aspectRatio: '1 / 1', overflow: 'hidden', borderRadius: '50%' }}>
                <img src={URL.createObjectURL(c.file)} alt="clothing" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="row" style={{ justifyContent: 'flex-end' }}>
          <button className="btn" type="submit" disabled={saving}>{saving ? 'Saving...' : 'Save & continue'}</button>
        </div>
      </form>
    </div>
  );
