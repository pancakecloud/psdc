...
import type { PinDoc, PinLabel } from "../services/pins";
import {
  createPin,
  deletePin,
  favoritePin,
...
export default function MapPage() {
  ...
  useEffect(() => {
    ...
    for (const p of pins) {
      if (!existing[p.id]) {
        ...
        m.on("mouseout", () =>
          setHoverPinId((id) => (id === p.id ? null : id))
        );
  ...
  const hoverPin = useMemo(
    () => pins.find((p) => p.id === hoverPinId) || null,
    [pins, hoverPinId]
  );
      ...
      >
        ...
        >
          ...
          <select
            ...
            style={{ maxWidth: 140 }}
            value={addingLabel}
            onChange={(e) => setAddingLabel(e.target.value as PinLabel)}
          >
        ...
        <div className="stack" style={{ marginBottom: 16 }}>
          <div className="info">
            Click on the map to add a {addingLabel} pin.
          </div>
        </div>
          ...
          <div className="stack">
            {myPins.map((p) => (
              ...
              >
                ...
                <button className="btn ghost" onClick={() => onDeletePin(p.id)}>
                  Delete
                </button>
              </div>
            ))}
      ...
      {hoverPin && (
        ...
        >
          <div style={{ fontWeight: 600, marginBottom: 4 }}>
            {hoverPin.label.toUpperCase()}
          </div>
          <div style={{ fontSize: 12, color: "var(--color-muted)" }}>
            Pinned by: {hoverPin.userId}
          </div>
          <div style={{ fontSize: 12, color: "var(--color-muted)" }}>
            Coords: {hoverPin.lat.toFixed(4)}, {hoverPin.lng.toFixed(4)}
          </div>
          <div className="row" style={{ marginTop: 8 }}>
            <button className="btn" onClick={() => onToggleFavorite(hoverPin)}>
              {isFav(hoverPin.id) ? "Unsave" : "Save"}
            </button>
          </div>
        </div>
      )}
